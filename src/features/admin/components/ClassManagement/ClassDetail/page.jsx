...
export default function ClassDetailPage() {
  ...
  const [isTeacherDetailModalOpen, setIsTeacherDetailModalOpen] =
    useState(false);
  ...
  useEffect(() => {
    const fetchClassData = async () => {
      try {
        ...
        try {
          const teacherResponse = await ClassGroupService.getHomeroomTeacher(
            id,
            token
          );
  ...
  const handleAddStudent = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await ClassGroupService.addStudent(
        id,
        newStudent.email,
        token
      );
  ...
  const handleDeleteStudent = async (studentId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await ClassGroupService.removeStudent(
        id,
        studentId,
        token
      );
  ...
  const handleAssignTeacher = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await ClassGroupService.assignHomeroomTeacher(
        id,
        newTeacher.email,
        token
      );
      ...
      if (response && response.data && response.data.success) {
        ...
        toast.success(
          response.data.message || "Thêm giáo viên chủ nhiệm thành công!"
        );
      } else {
        // Backend handles all validation logic, just display the error message
        const errorMessage =
          response?.data?.message ||
          response?.message ||
          "Không thể thêm giáo viên chủ nhiệm";
  ...
  const handleRemoveTeacher = async () => {
    try {
      ...
      if (response && response.data !== undefined) {
        ...
        toast.error(
          response?.message || "Không thể bỏ gán giáo viên chủ nhiệm"
        );
      }
    } catch (error) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription className="text-sm text-gray-600">
                  Thêm học sinh vào lớp {classData.name}. Nhập email học sinh để
                  thêm vào lớp.
                </DialogDescription>
              </div>
            </div>
          ...
          <div className="space-y-6 py-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                ...
                <br />
                <span className="text-orange-600 font-medium">Lưu ý:</span> Học
                sinh không thể có trong nhiều lớp cùng niên khóa.
              </p>
            </div>
          </div>
          ...
          <DialogHeader className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription className="text-sm text-gray-600">
                  Thêm giáo viên làm chủ nhiệm lớp {classData.name}. Nhập email
                  giáo viên để gán làm chủ nhiệm.
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="space-y-6 py-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                ...
                <br />
                <span className="text-orange-600 font-medium">Lưu ý:</span> Giáo
                viên không thể chủ nhiệm nhiều lớp cùng niên khóa.
              </p>
            </div>
          </div>
            ...
            <div className="py-4">
              <div className="space-y-4 text-sm">
                ...
                <div>
                  ...
                  <p className="text-gray-900 mt-1">
                    {homeroomTeacher.phoneNumber || "Chưa cập nhật"}
                  </p>
                </div>
              </div>
            </div>
