...
async getAllReport(token,PageNumber,PageSize,search){
    let url = `${API_CONFIG.MODERATION.GET_ALL_MODERATION}?PageNumber=${PageNumber}&PageSize=${PageSize}`;
...
async createReport(payload, token) {
    return this.post(API_CONFIG.MODERATION.GET_ALL_MODERATION, payload, token);
  }
