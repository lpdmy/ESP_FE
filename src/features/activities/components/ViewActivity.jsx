    ...
    const fetchActivity = async () => {
      ...
      try {
        ...
        if (response?.data) {
          ...
          // Check if user is already registered from participants list
          if (user?.id && activityData?.participants) {
            const userRegistered = activityData.participants.some(
              (participant) => 
    ...
    handleAction(async () => {
      ...
      try {
        ...
        if (response?.data) {
          ...
          const refreshResponse = await executeApiCall(
            activityService.getActivityById.bind(activityService),
            [params.id, token],
            { setError }
          )
          if (refreshResponse?.data) {
            const activityData = refreshResponse.data?.data || refreshResponse.data
            if (activityData) {
              setActivity(prev => ({
                ...prev,
                currentParticipants: activityData?.numberOfParticipants || prev?.currentParticipants || 0,
                participants: activityData?.participants || prev?.participants || [],
              }))
            }
          }
        }
