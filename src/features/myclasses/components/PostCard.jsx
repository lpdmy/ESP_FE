...
import { Heart, Download, FileText, Image, Video, Calendar, User, Edit, Trash2, MoreHorizontal } from 'lucide-react'
...
export default function PostCard({ post, onPostUpdated, canEdit = false, onEdit, onDelete }) {
  ...
  return (
    <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm">
      ...
      <CardContent className="space-y-4">
        ...
        {((post.attachments && post.attachments.length > 0) || (post.attachmentUrls && post.attachmentUrls.length > 0)) && (
                ...
                post.attachments.map((attachment, index) => (
                  <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => handleDownload(attachment.url, attachment.fileName || `attachment_${index + 1}`)}
                ...
                post.attachmentUrls.map((url, index) => (
                  <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => handleDownload(url, `attachment_${index + 1}`)}
                      className="h-8 w-8 p-0"
                    >
                      <Download className="h-4 w-4" />
                    </Button>
