...
export default function ClassManagementPage() {
  ...
  const loadHomeroomTeacher = async (classId) => {
    try {
      ...
      const response = await ClassGroupService.getHomeroomTeacher(
        classId,
        token
      );
      ...
      console.error(
        `Failed to load homeroom teacher for class ${classId}:`,
        error
      );
  ...
  useEffect(() => {
    ...
    const doSearch = async () => {
      try {
        const token = localStorage.getItem("token");
        const res = await ClassGroupService.search(
          {
            nameOrCombined: searchTerm,
    ...
    const loadClassWithSchedules = async () => {
      try {
        ...
        setNewClass({
          ...
          schedules: fullClass.schedules
            ? fullClass.schedules.map((s) => {
  ...
  const handleCreateClass = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        schedules:
          newClass.schedules && newClass.schedules.length > 0
  ...
  const handleUpdateClass = async () => {
    ...
    try {
      ...
      const res = await ClassGroupService.update(
        selectedClass.id,
        payload,
        token
      );
    ...
    <div className="space-y-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600">
          Quản lý danh sách lớp học theo năm học và khối
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <SimpleSelect
            value={
              selectedAcademicYearId ? selectedAcademicYearId.toString() : ""
            }
            onValueChange={(value) =>
        ...
        <Button
          ...
          className="bg-blue-500 hover:bg-blue-600 text-white shadow-sm rounded-md px-4"
        >
            ...
            <div>
              <h3 className="text-lg font-semibold text-blue-900">
                ...
                {getSelectedAcademicYearName()
                  ? `năm học ${getSelectedAcademicYearName()}`
                  : "tất cả niên khóa"}
              </h3>
              ...
              {isExpanded && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {gradeClasses.map((cls) => (
                    <Card
                      key={cls.id}
                      className="bg-white border-gray-200 shadow-sm rounded-2xl hover:shadow-md transition-shadow"
                    >
                      <CardContent className="p-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-600">
                              {homeroomTeachers[cls.id]
                                ? `${homeroomTeachers[cls.id].firstName} ${
                                    homeroomTeachers[cls.id].lastName
                                  }`.trim()
                                : "Chưa có GVCN"}
                            </span>
                            {!homeroomTeachers[cls.id] && (
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  navigate(
                                    `/admin/classes/${cls.id}?assignTeacher=true`
                                  );
                                }}
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[900px] bg-white max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Tạo lớp học mới. Điền thông tin bắt buộc bên dưới.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <div className="col-span-3">
                <SimpleSelect
                  ...
                  onValueChange={(value) =>
                    setNewClass({ ...newClass, grade: parseInt(value) })
                  }
            ...
            <div className="col-span-4">
              <SimpleClassScheduleEditor
                ...
                onChange={(schedules) =>
                  setNewClass({ ...newClass, schedules })
                }
              />
            </div>
          </div>
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[900px] bg-white max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Cập nhật thông tin lớp học. Điền thông tin bắt buộc bên dưới.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="edit-academicYear" className="text-right">
                Năm học *
              </Label>
              <div className="col-span-3">
                <SimpleSelect
      ...
      <Dialog open={isConfirmModalOpen} onOpenChange={setIsConfirmModalOpen}>
        <DialogContent className="sm:max-w-[425px] bg-white">
          <DialogHeader>
            ...
            <DialogDescription>
              Bạn có chắc chắn muốn xóa lớp học này? Hành động này không thể
              hoàn tác.
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 mt-1">
                      {getGradeName(classToDelete.grade)}
                    </p>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700">Năm học:</span>
                    <p className="text-gray-900 mt-1">
                      {getAcademicYearName(classToDelete.academicYearId)}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-gray-900 mt-1">
                      {classToDelete && homeroomTeachers[classToDelete.id]
                        ? `${homeroomTeachers[classToDelete.id].firstName} ${
                            homeroomTeachers[classToDelete.id].lastName
                          }`.trim()
                        : "Chưa có GVCN"}
                    </p>
                  </div>
