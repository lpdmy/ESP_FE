...
export default function VolleyballScoreboard({ data }) {
  // data: { teamA, teamB, sets: [{a,b}], faultsA, faultsB, teamALogo, teamBLogo, date, location, tournament }
  const { teamA, teamB, sets = [], faultsA = 0, faultsB = 0, teamALogo, teamBLogo, date, location, tournament } = data;
    ...
    <div className="space-y-6">
            ...
            <div className="relative">
              {teamALogo ? (
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden bg-white shadow-xl ring-4 ring-orange-100 flex items-center justify-center">
                  <img src={teamALogo} alt={teamA} className="w-full h-full object-contain p-1" />
                </div>
              ) : (
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-gradient-to-br from-orange-400 via-orange-500 to-orange-600 flex items-center justify-center text-white font-bold text-xl md:text-2xl shadow-xl ring-4 ring-orange-100">
                  {getTeamInitials(teamA)}
                </div>
              )}
              <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-green-500 rounded-full border-2 border-white"></div>
            </div>
            <div className="text-xl md:text-2xl font-bold text-gray-900">{teamA}</div>
          </div>
            ...
            <div className="relative">
              {teamBLogo ? (
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden bg-white shadow-xl ring-4 ring-blue-100 flex items-center justify-center">
                  <img src={teamBLogo} alt={teamB} className="w-full h-full object-contain p-1" />
                </div>
              ) : (
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-gradient-to-br from-blue-400 via-blue-500 to-blue-600 flex items-center justify-center text-white font-bold text-xl md:text-2xl shadow-xl ring-4 ring-blue-100">
                  {getTeamInitials(teamB)}
                </div>
              )}
              <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-green-500 rounded-full border-2 border-white"></div>
            </div>
            <div className="text-xl md:text-2xl font-bold text-gray-900">{teamB}</div>
          </div>
        </div>
      ...
      {sets && sets.length > 0 && (
        <div className="border-t border-gray-100 pt-6">
          <h4 className="text-base font-semibold text-gray-900 mb-4">Chi tiết các hiệp</h4>
          <div className="grid grid-cols-3 gap-4">
            {sets.map((set, i) => (
              <div key={i} className="bg-gray-50 rounded-xl p-4 text-center">
                <div className="text-xs font-medium text-gray-500 mb-2 uppercase tracking-wide">Hiệp {i + 1}</div>
                <div className="text-2xl font-bold text-gray-900">
                  {set.a} - {set.b}
                </div>
                <div className="text-xs text-gray-400 mt-1">
                  {set.a > set.b ? teamA : teamB} thắng
                </div>
              </div>
            ))}
          </div>
      ...
      {(faultsA > 0 || faultsB > 0) && (
        <div className="border-t border-gray-100 pt-6">
          ...
          <div className="flex items-center justify-center gap-8">
            <div className="text-center">
              <div className="text-sm text-gray-500 mb-1">{teamA}</div>
              <div className="text-2xl font-bold text-gray-900">{faultsA}</div>
            </div>
            <div className="text-gray-400">-</div>
            <div className="text-center">
              <div className="text-sm text-gray-500 mb-1">{teamB}</div>
