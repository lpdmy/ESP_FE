...
export function useProfileApi() {
  ...
  const getMyProfileRef = useRef(async () => {
    ...
    return executeApiCall(userService.getMyProfile.bind(userService), [token], {
  ...
  const updateMyPersonalInfoRef = useRef(async (payload) => {
    ...
    return executeApiCall(
      userService.updateMyPersonalInfo.bind(userService),
      [payload, token],
  ...
  const getMyTeacherProfileRef = useRef(async () => {
    const token = localStorage.getItem("token");
    return executeApiCall(
      userService.getMyTeacherProfile.bind(userService),
      [token],
      { setLoading: setProfileLoading, setError }
    );
  });
  ...
  const updateMyTeacherProfileRef = useRef(async (payload) => {
    const token = localStorage.getItem("token");
    return executeApiCall(
      userService.updateMyTeacherProfile.bind(userService),
      [payload, token],
      { setLoading: setSaveLoading, setError }
    );
  });
  const getStudentProfileRef = useRef(async (id) => {
    const token = localStorage.getItem("token");
    return executeApiCall(
      userService.getStudentProfileById.bind(userService),
      [id, token],
      { setLoading: setProfileLoading, setError }
    );
  });
  ...
  const getTeacherProfileRef = useRef(async (id) => {
    const token = localStorage.getItem("token");
    return executeApiCall(
      userService.getTeacherProfileById.bind(userService),
      [id, token],
      { setLoading: setProfileLoading, setError }
    );
  });
